...
class Deck:
    ...
    def dealHandGivenStrings(self, hand: list[str]) -> Hand:
        cards = []
        for i in range(3):
            cardStr = hand[i]
            cardId = self.cardIdByStr[cardStr]
            ...
            cards.append(Card(cardStr, cardId, cardValue, cardSuit, cardTrucoValue))

        return Hand(cards[0], cards[1], cards[2])

    def getCardByStr(self, cardStr: str) -> Card:
        cardId = self.cardIdByStr[cardStr]
        cardValue = self.cardValueByStr[cardStr]
        cardSuit = self.cardSuitByStr[cardStr]
        cardTrucoValue = self.cardTrucoValueByStr[cardStr]
        return Card(cardStr, cardId, cardValue, cardSuit, cardTrucoValue)
...
class Game:
    ...
    ACTION_TO_TAKE_IN_CASE_OF_INVALID = {
        0: 1,
        1: 2,
        2: 9,
        3: 0,
    ...
    def __init__(self, initialPointsP1, initialPointsP2, controlsP1):
        self.gameEnded = False
        self.puntosPartidaP1 = initialPointsP1
        self.puntosPartidaP2 = initialPointsP2
        self.controlsP1 = controlsP1
        self.resetRound()
    ...
    def resetRound(self):
        ...
        if(self.wantsToHardCodeHand==1):
            ...
            knownHandStrings = []
            for i in range(3):
                card = input(f"input card{i+1}: ")
                knownHandStrings.append(card)
            if(self.controlsP1):
                self.handP1 = self.DECK.dealHandGivenStrings(knownHandStrings)
            else:
                self.handP2 = self.DECK.dealHandGivenStrings(knownHandStrings)
        else:
            if(self.controlsP1):
    ...
    def invalidActionFeedback(self, actionDone):
        #override of invalid actions
        print("Overriding invalid action")
        if(self.isP1Turn):
            return self.handleActionP1(self.ACTION_TO_TAKE_IN_CASE_OF_INVALID[actionDone])
        else:
            return self.handleActionP2(self.ACTION_TO_TAKE_IN_CASE_OF_INVALID[actionDone])
    ...
    def throwCardManual(self, cardThrown):
        #ONLY USED WHEN NOT CONTROLLING THE PLAYER
        if(self.isP1Turn):
            #envido
            ...
            #truco
            self.cartasEnMesa[2*self.currentRound] = cardThrown
            self.stateP1[
                self.STATE_SECTIONS_INDECES["compartidoSection"] +
            ...
            self.pasteCardState(cardThrown, self.stateP1, 
                self.STATE_SECTIONS_INDECES["compartidoSection"] +
                self.STATE_COMPARTIDO_INDECES[f"carta{2*self.currentRound+1}"]
            )
            self.pasteCardState(cardThrown, self.stateP2, 
                self.STATE_SECTIONS_INDECES["compartidoSection"] +
                self.STATE_COMPARTIDO_INDECES[f"carta{2*self.currentRound+1}"]
            )
            ...
            #truco
            self.cartasEnMesa[2*self.currentRound+1] = cardThrown
            self.stateP1[
                self.STATE_SECTIONS_INDECES["compartidoSection"] +
    ...
    def throwCard(self, cardIdx):
        ...
        if(self.isP1Turn):
            ...
            print("P1 tira carta ", self.handP1[cardIdx])
            self.stateP1[
                self.STATE_SECTIONS_INDECES["compartidoSection"] +
                self.STATE_COMPARTIDO_INDECES[f"carta{2*self.currentRound+1}EnMesa"]
            ] = 1
            ...
            #truco
            print("P2 tira carta ", self.handP2[cardIdx])
    ...
    def handleActionP1(self, action):
        if(action==-1):
            #ONLY USE WHEN NOT CONTROLLING THIS PLAYER
            cardStr = input("Input card thrown: ")
            card = self.DECK.getCardByStr(cardStr)
            self.throwCardManual(card)

        if(action==0):
            #tirarC1
            if(not self.isP1TrucoTurn): return self.invalidActionFeedback(action)
            if(self.cardWasUsedP1[0]): return self.invalidActionFeedback(action)
            if(self.truco_P1HasToRespondBet): return self.invalidActionFeedback(action)
            if(self.envido_P1HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action==1):
            #tirarC2
            if(not self.isP1TrucoTurn): return self.invalidActionFeedback(action)
            if(self.cardWasUsedP1[1]): return self.invalidActionFeedback(action)
            if(self.truco_P1HasToRespondBet): return self.invalidActionFeedback(action)
            if(self.envido_P1HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action==2):
            #tirarC3
            if(not self.isP1TrucoTurn): return self.invalidActionFeedback(action)
            if(self.cardWasUsedP1[2]): return self.invalidActionFeedback(action)
            if(self.truco_P1HasToRespondBet): return self.invalidActionFeedback(action)
            if(self.envido_P1HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action==3):
            #Acepta envido
            if(not self.envido_P1HasToRespondBet): return self.invalidActionFeedback(action)
            ...
            print("++++")
            print("Deciding who wins envido...")
            #MAL SI QUERES JUGAR ONLINE
            puntosP1 = self.handP1.envidoValueOfHand
            puntosP2 = self.handP2.envidoValueOfHand
            if (puntosP1 < puntosP2):
                P2Wins = 1
            else:
                P2Wins = 0
            print(f"P1 envido points: {puntosP1}")
            if(P2Wins==0):
                ...
                self.stateP1[
                    ...
                    self.ENVIDO_POINTS_INDECES[puntosP1]
                ] = 1
                self.stateP2[
                    self.STATE_SECTIONS_INDECES["compartidoSection"] +
                    self.STATE_COMPARTIDO_INDECES["envidoPuntosPublicosP1"] +
                ...
                print(f"P2 envido points: {puntosP2}")
                print("P2 won envido")
                ...
                self.stateP1[
                    ...
                    self.ENVIDO_POINTS_INDECES[puntosP2]
                ] = 1
                self.stateP2[
                    self.STATE_SECTIONS_INDECES["compartidoSection"] +
                    self.STATE_COMPARTIDO_INDECES["envidoPuntosPublicosP2"] +
        ...
        if(action==5):
            #Envido
            if(self.envido_alreadyPlayed): return self.invalidActionFeedback(action)
            if(not self.envido_P1CanBet): return self.invalidActionFeedback(action)
            if(not self.envido_P1ValidBets[0]): return self.invalidActionFeedback(action)
        ...
        if(action==6):
            ...
            if(self.envido_alreadyPlayed): return self.invalidActionFeedback(action)
            if(not self.envido_P1CanBet): return self.invalidActionFeedback(action)
            if(not self.envido_P1ValidBets[1]): return self.invalidActionFeedback(action)
        ...
        if(action==7):
            ...
            if(self.envido_alreadyPlayed): return self.invalidActionFeedback(action)
            if(not self.envido_P1CanBet): return self.invalidActionFeedback(action)
            if(not self.envido_P1ValidBets[2]): return self.invalidActionFeedback(action)
        ...
        if(action == 8):
            #aceptar Truco bet
            if(self.envido_P1HasToRespondBet): return self.invalidActionFeedback(action)
            if(not self.truco_P1HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action == 10):
            #bet Truco
            if(not self.truco_P1CanBet): return self.invalidActionFeedback(action)
            if(self.envido_P1HasToRespondBet): return self.invalidActionFeedback(action)
    ...
    def handleActionP2(self, action):
        ...
        if(action==0):
            #tirarC1
            if(self.isP1TrucoTurn): return self.invalidActionFeedback(action)
            if(self.cardWasUsedP2[0]): return self.invalidActionFeedback(action)
            if(self.truco_P2HasToRespondBet): return self.invalidActionFeedback(action)
            if(self.envido_P2HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action==1):
            #tirarC2
            if(self.isP1TrucoTurn): return self.invalidActionFeedback(action)
            if(self.cardWasUsedP2[1]): return self.invalidActionFeedback(action)
            if(self.truco_P2HasToRespondBet): return self.invalidActionFeedback(action)
            if(self.envido_P2HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action==2):
            #tirarC3
            if(self.isP1TrucoTurn): return self.invalidActionFeedback(action)
            if(self.cardWasUsedP2[2]): return self.invalidActionFeedback(action)
            if(self.truco_P2HasToRespondBet): return self.invalidActionFeedback(action)
            if(self.envido_P2HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action==3):
            #Acepta envido
            if(not self.envido_P2HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action==5):
            #Envido
            if(self.envido_alreadyPlayed): return self.invalidActionFeedback(action)
            if(not self.envido_P2CanBet): return self.invalidActionFeedback(action)
            if(not self.envido_P2ValidBets[0]): return self.invalidActionFeedback(action)
        ...
        if(action==6):
            ...
            if(self.envido_alreadyPlayed): return self.invalidActionFeedback(action)
            if(not self.envido_P2CanBet): return self.invalidActionFeedback(action)
            if(not self.envido_P2ValidBets[1]): return self.invalidActionFeedback(action)
        ...
        if(action==7):
            ...
            if(self.envido_alreadyPlayed): return self.invalidActionFeedback(action)
            if(not self.envido_P2CanBet): return self.invalidActionFeedback(action)
            if(not self.envido_P2ValidBets[2]): return self.invalidActionFeedback(action)
        ...
        if(action == 8):
            #aceptar Truco bet
            if(self.envido_P2HasToRespondBet): return self.invalidActionFeedback(action)
            if(not self.truco_P2HasToRespondBet): return self.invalidActionFeedback(action)
        ...
        if(action == 10):
            #bet Truco
            if(not self.truco_P2CanBet): return self.invalidActionFeedback(action)
            if(self.envido_P2HasToRespondBet): return self.invalidActionFeedback(action)
